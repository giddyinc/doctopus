...
class Doc {
  ...
  paramGroup(name) {
    ...
    setImmediate(() => {
      ...
      if (schema) {
        Object.values(schema).forEach(param => self.param(param));
      }
    });
    return this;
  }
   ...
   * Must be sync because this is scoped to many docs.
  ...
  setMethod(methodName) {
    const clone = new Doc(_.cloneDeep(this.doc));
    if (Object.keys(clone.doc).length === 0) {
      clone.doc[methodName] = {};
    } else {
      const old = Object.keys(clone.doc)[0];
      if (methodName === old) {
        return clone;
      }
      clone.doc[methodName] = clone.doc[old];
      delete clone.doc[old];
    }
    return clone;
  }
  ...
  build() {
    ...
    return new Promise(resolve => {
      setImmediate(() => {
        self.setParamsSync(Object.values(self._params));
        return resolve(self.doc);
      });
    });
  }
